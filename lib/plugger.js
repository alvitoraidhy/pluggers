...
class Plugger {
  ...
  addPlugin(param) {
    ...
    if (!Object.keys(this[plugins]).includes(name)) {
      ...
      for (let x = 0, len = required.length; x < len; x += 1) {
        if (typeof this[plugins][required[x]] === 'undefined') {
          throw new RequirementError(`Required plugin is not loaded -> '${required[x]}' (required by '${name}')`);
        }
      }
  ...
  removeRequiredPlugin(name) {
    if (!isString(name)) {
      throw new TypeError(`${name} is not a string`);
    } else if (this[requredPlugins].indexOf(name) > -1) {
