...
(async () => {
  ...
  await Promise.all(Object.keys(groupedVersions).map((x) => 
    Promise.all(groupedVersions[x].slice(1).map((x => 
  ...
  await Promise.all(files
    ...
    .map(async (file) => {
      ...
      return await fs.promises.writeFile(outFilePath, str).then(() =>
        console.log(`Rendered file: ${file} -> ${outFilePath}`)
      );
    }));
